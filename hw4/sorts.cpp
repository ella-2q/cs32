...
void sortUsingPtrs(vector<Sensor>& sensors, bool comp(const Sensor*, const Sensor*))
{
      ...
      // Create an auxiliary copy of sensors to facilitate the later reordering.
    ...
    for (vector<Sensor>::iterator p = auxSensors.begin(); p != auxSensors.end(); p++){
        sensorptr.push_back(&(*p));
    }

      // TODO:  Sort the vector of pointers using the STL sort algorithm
      //        with the comp parameter as the ordering relationship.
    
    sort(sensorptr.begin(), sensorptr.end(), comp);
    ...
    for (size_t i = 0; i < sensors.size(); i++) {
        sensors[i] = *(sensorptr[i]);  // Copy the Sensor that the pointer points to
    }
